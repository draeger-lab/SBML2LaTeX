/*
 * $Id: LaTeXReportGenerator.java 60 2011-03-07 17:20:39Z draeger $
 * $URL: https://rarepos.cs.uni-tuebingen.de/svn/SBML2LaTeX/tags/version0.9.8/src/org/sbml/tolatex/io/LaTeXReportGenerator.java $
 * ---------------------------------------------------------------------
...
public class LaTeXReportGenerator extends LaTeX implements SBMLReportGenerator {
     ...
     */
    private static MIRIAMparser miriam;
	...
	try {
	    miriam = SBML2LaTeX.getMIRIAMparser();
	} catch (Exception e) {
	    e.printStackTrace();
	}
    }
     ...
     * together
     * with the respective name. If false (default), a subsection for each one
     * of
     * the three groups of participants is created giving all details of each
     * participant.
     */
    private boolean arrangeReactionParticipantsInOneTable = false,  printFullODEsystem = false;
     ...
     * and returns the filled list of function identifiers. Every function
     * called
     * in this tree is added only one time to the list (in the order the
     * functions
     * are called).
     * 
     * @param ast
	...
	if ((math.getType() == ASTNode.Type.NAME)
		&& (math.getName().equals(id))) {
	    return true;
	}
	...
	if (list.size() > 0) {
		...
		for (i = 0; i < list.size(); i++) {
			...
			((QuantityWithUnit) list.get(i)).getValue())
			    .split("\\.");
		...
		if (parameters) {
			...
			if (p.isSetUnits()) {
			    ...
			    } else if (Unit.isPredefined(p.getUnits(), p
				    .getLevel())) {
		    ...
		    buffer.append('&');
		    buffer.append(StringTools.toString(Locale.ENGLISH, c.getSpatialDimensions()));
		    buffer.append('&');
		    ...
		    buffer.append('&');
		    buffer
			    .append(math(format(spec.getDerivedUnitDefinition())));
		    buffer.append('&');
		    ...
		    buffer.append(descriptionItem("Equation",
			...
			for (j = 0; j < s.getModel().getNumSpecies(); j++) {
			    ...
			    if (spec.isSetSpeciesType()
				    && spec.getSpeciesType()
					    .equals(nsb.getId())) {
				sb.append(texttt(maskSpecialChars(spec
						.getId())));
				sb.append('&');
				if (spec.isSetName()) {
	...
	if (s instanceof NamedSBase) {
	    NamedSBase nsb = (NamedSBase) s;
	    buffer.append(maskSpecialChars(nsb.isSetName() ? nsb.getName()
		    : def));
	}
	buffer.append(def);
    }
	...
	if (eventList.size() > 0) {
		...
		if (ev.isSetId()) {
		    ...
		    buffer.append(subsection("Event without an identifier",
			true));
		...
		if (ev.isSetDelay()) {
		    buffer.append(descriptionItem("Delay", equation(ev
			    .getDelay().getMath().toLaTeX())));
		...
		if (ev.getUseValuesFromTriggerTime()) {
		    ...
		    description.append(1 < ev.getNumEventAssignments() ? "are"
			    : "is");
		...
		for (j = 0; j < ev.getNumEventAssignments(); j++) {
			...
			description.append(mathtt(maskSpecialChars(model
		    ...
		    description
			    .append((ev.getNumEventAssignments() > 1) ? " =& "
				    : " = ");
		    description.append(ev.getEventAssignment(j).getMath()
			    .toLaTeX());
	...
	ListOf<UnitDefinition> lud = new ListOf<UnitDefinition>(m.getLevel(), m
		.getVersion());
	...
	if (model.isSetSBOTerm()) {
	    ...
	    String sboModelName = maskSpecialChars(correctQuotationMarks(SBO
		    .getTerm(model.getSBOTerm()).getName(), leftQuotationMark,
		rightQuotationMark));
	...
	buffer.append(" an overview of the quantities of all components of this model.");
	buffer.newLine();
	...
	buffer.append("\\multicolumn{1}{c}{Element}&\\multicolumn{1}{|c||}{Quantity}&");
	buffer.append("\\multicolumn{1}{c|}{Element}&\\multicolumn{1}{c}{Quantity}");
	buffer.append(lineBreak);
	...
	if ((history.getNumCreators() > 0) || (history.isSetCreatedDate())) {
	    buffer
		    .append(String.format("This %s was ", sbase
			    .getElementName()));
	...
	if (!u.isDimensionless()) {
		...
		prefix = u.getScale() != 0 ? String.format("10^{%s}", Integer
			.toString(u.getScale())) : "";
     ...
     * of
     * the given SBML file and writes all errors and warnings found to at the
     * end
     * of the document.
     * 
     * @return
     ...
     *        small tables, false if a subsection should be created for the
     *        three
     *        groups.
     */
	...
	else System.err.println("Unsupported font " + fontTypewriter
	...
	if (checkConsistency) {
		...
		if (notImplemented) {
		    ...
		    buffer
			    .append("library \\href{http://jsbml.sourceforge.net}");
		    buffer.append("{JSBML}, which is used ");
		    buffer
			    .append("by the documentation tool \\SBMLLaTeX, does ");
		    buffer
			    .append("not provide an implementation for full SBML ");
		    buffer
			    .append("consistency checks. Hence, at this position ");
		    ...
		    buffer
			    .append("{sbml.org} and use the SBML validator there.");
		} else {
		    ...
		    buffer
			    .append(" listed in the remainder of this model report.");
		    buffer.newLine();
		    buffer
			    .append("The messages and identification codes shown ");
	...
	if (sboTerms.size() > 0) {
		...
		if (term != null) {
		    ...
		    def = maskSpecialChars(correctQuotationMarks(def,
			leftQuotationMark, rightQuotationMark));
		    name = maskSpecialChars(correctQuotationMarks(term
			    .getName(), leftQuotationMark, rightQuotationMark));
		    buffer.append(descriptionItem(String.format("%s", term
			    .getId()), String.format("\\textbf{%s:} %s%s",
			name, def, newLine())));
		}
	    }
	    buffer.append(descriptionEnd);
	...
	for (int i = 0; i < resources.size(); i++) {
		...
		if (urls.length >= 1) {
			...
			.append(" no URL available for resource identifier "
				+ texttt(identifier));
	    }
	    buffer.newLine();
	}
	...
	if ((def.getNumArguments() > 0) || (def.getBody() != null)
		...
		buffer.append(descriptionItem("Mathematical Formula", equation(
		    getNameOrID(def, true), new StringBuffer(def.getMath()
			    .toLaTeX()))));
	    }
	}
	buffer.append(descriptionEnd);
	...
	for (int i = 0; i < list.size(); i++) {
		...
		String dimension = (spatialDim - c.getSpatialDimensions() == 0d) ? getWordForNumber(spatialDim)
					: StringTools.toString(Locale.ENGLISH, c
	...
	if (model.getNumSpecies() > 0) {
		...
		if (species.isSetInitialConcentration()) {
			...
			if (species.isSetSubstanceUnits()
			    ...
			    ud = new UnitDefinition(species.getLevel(), species
				    .getVersion());
			    if (Unit.isUnitKind(species.getSubstanceUnits(),
				species.getLevel(), species.getVersion())) {
				Unit u = new Unit(species.getLevel(), species
					.getVersion());
				u.setKind(Unit.Kind.valueOf(species
					.getSubstanceUnits()));
				ud.addUnit(u);
			    }
		...
		for (i = 0; i < model.getNumEvents(); i++) {
			...
			if (event.getEventAssignment(j).getVariable().equals(
			    species.getId())) {
			    eventsInvolved.add(event.isSetId() ? event.getId()
				    : Integer.toString(i));
			}
		    }
		}
		...
		if (rulesInvolved.size() > 0) {
			...
			buffer
				.append(hyperref("rule" + index, "Rule "
					+ index));
		...
		for (i = 0; i < productsReaction[(int) speciesIndex].size(); i++) {
		    ...
		    SpeciesReference product = r.getProductForSpecies(species
			    .getId());
		...
		for (i = 0; i < reactantsReaction[(int) speciesIndex].size(); i++) {
		    ...
		    SpeciesReference reactant = r.getReactantForSpecies(species
			    .getId());
		    equationBW.append('-');
		...
		if (species.getBoundaryCondition()) {
			...
			if (rulesInvolved.size() > 0) {
			    ...
			    for (Iterator<Integer> iterator = rulesInvolved
	...
	if (arrangeReactionParticipantsInOneTable) {
		...
		for (i = 0; i < r.getNumReactants(); i++) {
			...
			reactString.append(texttt(maskSpecialChars(reactant
				.getSpecies())));
			reactString.append(" (");
			reactString.append(SBO.sboNumberString(reactant
				.getSBOTerm()));
		...
		for (i = 0; i < r.getNumProducts(); i++) {
			...
			reactString.append(texttt(maskSpecialChars(product
				.getSpecies())));
			reactString.append(" (");
			reactString.append(SBO.sboNumberString(product
				.getSBOTerm()));
		...
		if (hasSBOModifiers > 0) {
			...
			if (m.isSetSBOTerm()) {
			    reactString.append(" (");
			    reactString.append(SBO.sboNumberString(m
				    .getSBOTerm()));
			    reactString.append(" ");
			    reactString
				    .append(maskSpecialChars(correctQuotationMarks(
					SBO.getTerm(m.getSBOTerm()).getName(),
					leftQuotationMark, rightQuotationMark)));
	...
	if (arrangeReactionParticipantsInOneTable) {
		...
		if (r.getNumReactants() > 0) {
			...
			/*
			 * reactString.append('&');
			 * reactString.append(s.isSetSBOTerm() ?
		...
		reactString.append(subsubsection(
		    r.getNumReactants() > 1 ? "Reactants" : "Reactant", false));
		...
		for (i = 0; i < r.getListOfReactants().size(); i++) {
			...
			reactString.append(SBO.sboNumberString(specRef
				.getSBOTerm()));
	    ...
	    if (r.getNumModifiers() > 0) {
		reactString.append(subsubsection(
		    r.getNumModifiers() > 1 ? "Modifiers" : "Modifier", false));
		reactString.append(longtableHead(columnDef, caption
			+ "modifier.", headLine));
		for (i = 0; i < r.getListOfModifiers().size(); i++) {
			...
			reactString.append(SBO.sboNumberString(modRef
				.getSBOTerm()));
	    ...
	    if (r.getNumProducts() > 0) {
		reactString.append(subsubsection(
		    r.getNumProducts() > 1 ? "Products" : "Product", false));
		reactString.append(longtableHead(columnDef, caption
			+ "product.", headLine));
	...
	if (r.getKineticLaw() != null) {
	    ...
	    if (r.getKineticLaw().getNumLocalParameters() > 0) {
		format(r.getKineticLaw().getListOfLocalParameters(), pBuffer,
		    false);
	    }
	    pBuffer.close();
	} else {
	...
	if (functionCalls != null) {
		...
		reactString.append(equation(mathtt(maskSpecialChars(id)),
		    new StringBuffer(r.getModel().getFunctionDefinition(id)
			    .getMath().toLaTeX())));
	    }
	}
	...
	if (rl.isAlgebraic()) {
		...
		buffer.append(" As this model is overdetermined, it is not possible to state, which variable is determined by this rule.");
	    }
		...
		hasOnlySubstanceUnits = rl.getModel().getSpecies(
		    ((Assignment) rl).getVariable()).getHasOnlySubstanceUnits();
	    } else if (rl.getModel().getCompartment(
		((Assignment) rl).getVariable()) != null) {
		buffer.append("compartment ");
	    } else {
		buffer.append("parameter ");
	    }
	    buffer.append(texttt(maskSpecialChars(((Assignment) rl)
		    .getVariable())));
	    buffer.append(':');
	    buffer.append(eqBegin);
	    buffer.append("\\frac{\\mathrm d}{\\mathrm dt} ");
		...
		buffer.append(mathtt(maskSpecialChars(((Assignment) rl)
			.getVariable())));
	    }
	    if (hasOnlySubstanceUnits) {
		buffer.append(']');
     ...
     * @param buffer
     * @param onlyItems
     *        If true items will be written otherwise this will be surrounded by
     *        a
	...
	if (((sBase instanceof NamedSBase) && ((NamedSBase) sBase).isSetName())
	    ...
	    if (((sBase instanceof NamedSBase) && ((NamedSBase) sBase)
		    .isSetName())) {
		...
		buffer.append(descriptionItem("SBO:"
				+ SBO.sboNumberString(sBase.getSBOTerm()),
			    correctQuotationMarks(SBO.getTerm(
				sBase.getSBOTerm()).getName(),
				leftQuotationMark, rightQuotationMark)));
	...
	List<Integer>[] modifierReaction, BufferedWriter buffer)
	throws IOException {
	...
	final int numReactionsInvolved = numReactants + numProducts
		+ numModifiers;
	...
	for (int i = 0, reactionIndex; i < numReactionsInvolved; i++) {
		...
		reactionIndex = reactantsReaction[speciesIndex].get(i)
			.intValue();
		...
		reactionIndex = productsReaction[speciesIndex].get(
		    i - numReactants).intValue();
	    } else {
     ...
     * @throws SBMLException 
     */
    private void formatVelocity(Reaction reaction, int k, boolean notSubstancePerTimeUnit,
     ...
     * of
     * the given SBase. If printNameIfAvailable is true this method looks for
     * the
	...
	// imprint.append("of America}, Lukas Endler\\footnote{European ");
	// imprint.append("Bioinformatics Institute, Wellcome Trust Genome ");
	// imprint.append("Campus, Hinxton, United Kingdom}, Martin Golebiewski");
	...
	for (Integer e : errors) {
	    ...
	    buffer
		    .append(descriptionItem(messageType + ' '
			    + Integer.toString(error.getErrorId()), message
			    .toString()));
	}
	buffer.append(descriptionEnd);
	buffer.newLine();
	...
	if (!SBFileFilter.isTeXFile(file)) {
	    throw new IOException(String.format("%s is not a valid TeX file.",
		file));
	}
