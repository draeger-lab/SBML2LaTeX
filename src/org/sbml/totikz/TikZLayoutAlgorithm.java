...
public class TikZLayoutAlgorithm extends SimpleLayoutAlgorithm {
  ...
  public Dimensions createCompartmentGlyphDimension(CompartmentGlyph previousCompartmentGlyph) {
    if (previousCompartmentGlyph == null) {
      ...
      return new Dimensions(width, depth, height, level, version);
    }
  }
  ...
  public Curve createCurve(ReactionGlyph reactionGlyph, SpeciesReferenceGlyph specRefGlyph) {
    ...
    curve.setVersion(version);

    ListOf<CurveSegment> curveSegmentsList = new ListOf<CurveSegment>(level, version);

    LineSegment curveSegment = new LineSegment();
    curveSegment.setLevel(level);
    curveSegment.setVersion(version);
    ...
    if (specRefGlyph.isSetSpeciesGlyph()) {
      ...
      Point rgCurvePoint = new Point(level, version);
      // curve point at species glyph
      Point sgCurvePoint = new Point(level, version);
  ...
  public Dimensions createLayoutDimension() {
    ...
    SpeciesGlyph leftMostGlyph = new SpeciesGlyph(level, version);
    leftMostGlyph.createBoundingBox(0, 0, 0, 0, 0, 0);

    SpeciesGlyph rightMostGlyph = new SpeciesGlyph(level, version);
    rightMostGlyph.createBoundingBox(0, 0, 0, 300, 300, 0);

    SpeciesGlyph topMostGlyph = new SpeciesGlyph(level, version);
    topMostGlyph.createBoundingBox(0, 0, 0, 0, 0, 0);

    SpeciesGlyph bottomMostGlyph = new SpeciesGlyph(level, version);
    ...
    return new Dimensions(width, height, 0, level, version);
  }
  ...
  public Dimensions createSpeciesGlyphDimension() {
    ...
    return new Dimensions(60, 60, 0, level, version);
  }
