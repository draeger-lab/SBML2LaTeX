...
public class SBML2LaTeX extends LaTeX implements libsbmlConstants, DisplaySBML {
	...
	public void format(ListOf list, BufferedWriter buffer, boolean section)
		...
		if (list.size() > 0) {
			...
			if (compartments)
				...
				buffer.append(" modifier, the ");
				if (printNameIfAvailable)
					buffer.append("name or---if not specified---the");
				buffer.append(" identifier of this species is");
	...
	private StringBuffer imprint() {
		...
		imprint.append("Hannes Planatscher$^a$, Dieudonn\\'e M Wouamba");
		imprint.append("$^a$, Adrian Schr\\\"oder$^a$, Michael Hucka");
		...
		imprint.append(" and Andreas Zell$^a$. Please see \\url{");
		imprint.append("http://www.ra.cs.uni-tuebingen.de/software/SBML2LaTeX");
		imprint.append("} for more information.");
		imprint.append(newLine);
