...
public class SBML2LaTeX extends LaTeX implements libsbmlConstants, DisplaySBML {
	...
	public StringBuffer toLaTeX(Model model, ASTNode astnode)
		...
		case AST_DIVIDE:
			return value = frac(toLaTeX(model, astnode.getLeftChild()), toLaTeX(model,
		...
		case AST_FUNCTION_ABS:
			return value = abs(toLaTeX(model, astnode
					.getChild(astnode.getNumChildren() - 1)));

		case AST_FUNCTION_ARCCOS:
			if (astnode.getLeftChild().getNumChildren() > 0)
				return value = arccos(brackets(toLaTeX(model, astnode.getLeftChild())));
			return value = arccos(toLaTeX(model, astnode.getLeftChild()));

		case AST_FUNCTION_ARCCOSH:
			if (astnode.getLeftChild().getNumChildren() > 0)
				return value = arccosh(brackets(toLaTeX(model, astnode.getLeftChild())));
		...
		case AST_FUNCTION_ROOT:
			...
			if ((astnode.getNumChildren() > 1)
				...
				return value = root(toLaTeX(model, astnode.getLeftChild()), toLaTeX(
