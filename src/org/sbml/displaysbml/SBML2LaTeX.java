...
public class SBML2LaTeX extends LaTeX implements libsbmlConstants, DisplaySBML {
	 ...
	 * ASTNode and returns the filled list of function identifiers. Every
	 * function called in this tree is added only one time to the list (in the
	...
	public void format(ListOfEvents eventList, BufferedWriter buffer)
		...
		if (eventList.size() > 0) {
			...
			for (i = 0; i < eventList.size(); i++) {
				...
				buffer.append(descriptionItem("Trigger condition",
						equation(toLaTeX(ev.getModel(), ev.getTrigger()
								.getMath()))));
				if (ev.isSetDelay()) {
					buffer.append(descriptionItem("Delay", equation(toLaTeX(ev
							.getModel(), ev.getDelay().getMath()))));
				...
				for (j = 0; j < ev.getNumEventAssignments(); j++) {
					...
					description
							.append((ev.getNumEventAssignments() > 1) ? " =& "
									: " = ");
					description.append(toLaTeX(ev.getModel(), ev
	...
	public void format(ListOfUnitDefinitions listOfUnits, BufferedWriter buffer)
		...
		if (showPredefinedUnitDeclarations) {
			...
			if (listOfUnits.get("substance") == null) {
				def = new UnitDefinition(listOfUnits.getLevel(), listOfUnits
						.getVersion());
				def.setId("substance");
				u = new Unit(listOfUnits.getLevel(), listOfUnits.getVersion());
			...
			if (listOfUnits.get("volume") == null) {
				...
				def.setId("volume");
				u = new Unit(listOfUnits.getLevel(), listOfUnits.getVersion());
				u.setKind(libsbml.UnitKind_forName("litre"));
				def.addUnit(u);
			...
			if ((listOfUnits.get("area") == null)
				...
				def.setId("area");
				u = new Unit(listOfUnits.getLevel(), listOfUnits.getVersion());
				u.setKind(libsbml.UnitKind_forName("metre"));
				u.setExponent(2);
				def.addUnit(u);
			...
			if (listOfUnits.get("time") == null) {
				...
				def.setId("time");
				u = new Unit(listOfUnits.getLevel(), listOfUnits.getVersion());
				u.setKind(libsbml.UnitKind_forName("second"));
				def.addUnit(u);
	...
	public StringBuffer toLaTeX(Model model, ASTNode astnode)
		...
		if (astnode.isUMinus()) {
			...
			if (astnode.getLeftChild().getNumChildren() > 0
					&& astnode.getLeftChild().getType() != AST_TIMES)
			...
			return value = sqrt(toLaTeX(model, astnode.getChild(astnode
					.getNumChildren() - 1)));
		 ...
		 */
		case AST_REAL:
			return value = format(astnode.getReal());
		...
		case AST_DIVIDE:
			return value = frac(toLaTeX(model, astnode.getLeftChild()),
		...
		case AST_FUNCTION_ABS:
			return value = abs(toLaTeX(model, astnode.getChild(astnode
					.getNumChildren() - 1)));

		case AST_FUNCTION_ARCCOS:
			if (astnode.getLeftChild().getNumChildren() > 0)
				return value = arccos(brackets(toLaTeX(model, astnode
						.getLeftChild())));
			return value = arccos(toLaTeX(model, astnode.getLeftChild()));

		case AST_FUNCTION_ARCCOSH:
			if (astnode.getLeftChild().getNumChildren() > 0)
				return value = arccosh(brackets(toLaTeX(model, astnode
						.getLeftChild())));
			return value = arccosh(toLaTeX(model, astnode.getLeftChild()));
		...
		case AST_FUNCTION_CEILING:
			return value = ceiling(toLaTeX(model, astnode.getLeftChild()));
		...
		case AST_FUNCTION_FLOOR:
			return value = floor(toLaTeX(model, astnode.getLeftChild()));
		...
		case AST_FUNCTION_ROOT:
			...
			if ((astnode.getNumChildren() > 1)
				...
				return value = root(toLaTeX(model, astnode.getLeftChild()),
						toLaTeX(model, astnode.getRightChild()));
			return value = sqrt(toLaTeX(model, astnode.getChild(astnode
					.getNumChildren() - 1)));
	...
	private void documentFoot(SBMLDocument doc, BufferedWriter buffer)
		...
		if (sboTerms.size() > 0) {
			...
			Arrays.sort(sbo);
			for (int id : sbo)
				buffer.append(descriptionItem("SBO:" + getSBOnumber(id),
						"\\textbf{"
								+ maskSpecialChars(
										correctQuotationMarks(SBOParser
												.getSBOTermName(id),
												leftQuotationMark,
												rightQuotationMark))
										.replaceAll("\\n", " ")
						...
						if (species.isSetSubstanceUnits()
							...
							if (Unit.isUnitKind(species.getSubstanceUnits(),
								...
								Unit u = new Unit(species.getLevel(), species
										.getVersion());
								u.setKind(libsbml.UnitKind_forName(species
										.getSubstanceUnits()));
								ud.addUnit(u);
							}
	...
	private StringBuffer format(Reaction r, long reactionIndex)
		...
		reactString.append(newLine);
		reactString.append(label("v" + Long.toString(reactionIndex + 1)));
	...
	private void formatReactionsInvolved(Model model, long speciesIndex,
		...
		for (long i = 0, reactionIndex; i < numReactionsInvolved; i++) {
			...
			if ((0 < i) && (!noComma)) {
				...
				buffer.append(' ');
			buffer.append(hyperref("v" + Long.toString(reactionIndex),
	...
	private void formatVelocity(Long k, boolean notSubstancePerTimeUnit,
		...
		v.append(Long.toString(k));
		v.append('}');
		if (notSubstancePerTimeUnit)
			v = colorbox("lightgray", math(v));
		else if (notExistingKineticLaw)
			v = textcolor("red", v);
		equationBW.append(hyperref("v" + Long.toString(k), v));
	}
