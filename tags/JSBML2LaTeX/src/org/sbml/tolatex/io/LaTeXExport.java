...
public class LaTeXExport extends LaTeX implements SBMLReporter {
	...
	public void format(ListOf<? extends SBase> list, BufferedWriter buffer,
		...
		int i;
		boolean compartments = list.getFirst() instanceof Compartment;
		boolean species = list.getFirst() instanceof Species;
		boolean reactions = list.getFirst() instanceof Reaction;
		boolean parameters = list.getFirst() instanceof Parameter;
		if (headTail) {
		...
		if (list.size() > 0) {
			...
			if (compartments) {
				...
				for (i = 0; i < list.size(); i++) {
					String[] value = Double
							.toString(((Parameter) list.get(i)).getValue()).split("\\.");
			...
			for (i = 0; i < list.size(); i++) {
				...
				if (parameters) {
					...
					if ((ud == null) || (ud.getNumUnits() == 0)) {
						if (p.isSetUnits()) {
							if ((ud = p.getModel().getUnitDefinition(p.getUnits())) != null) {
								...
								u.setKind(JSBML.forName(p.getUnits()));
					...
					buffer.append('&');
					buffer.append(Integer.toString(c.getSpatialDimensions()));
					buffer.append('&');
					...
					if (isSpecType)
						...
						if (c.isSetCompartmentType()
							...
							sb.append('&');
							if (c.isSetName()) sb.append(maskSpecialChars(c.getName()));
							sb.append(lineBreak);
							counter++;
						}
					}
	...
	private void formatEvents(ListOf<? extends SBase> eventList,
		...
		if (eventList.size() > 0) {
			...
			for (i = 0; i < eventList.size(); i++) {
				...
				buffer.append(descriptionItem("Trigger condition", equation(ev
						.getTrigger().getMath().toLaTeX())));
				if (ev.isSetDelay()) {
	...
	public void format(Model model, BufferedWriter buffer) throws IOException,
		...
		if (model.isSetSBOTerm()) {
			...
			String sboModelName = maskSpecialChars(correctQuotationMarks(SBO.getTerm(
				model.getSBOTerm()).getName(), leftQuotationMark, rightQuotationMark));
		...
		// Deprectated since Akiya's patch of libSBML 3.3.1
		...
		if (headTail) documentFoot(model.getSBMLDocument(), buffer);
	}
	...
	public StringBuffer format(Unit u) {
		...
		boolean standardScale = (u.getScale() == 18) || (u.getScale() == 12)
				|| (u.getScale() == 9) || (u.getScale() == 6) || (u.getScale() == 3)
				|| (u.getScale() == 2) || (u.getScale() == 1) || (u.getScale() == 0)
				|| (u.getScale() == -1) || (u.getScale() == -2) || (u.getScale() == -3)
				|| (u.getScale() == -6) || (u.getScale() == -9)
				|| (u.getScale() == -12) || (u.getScale() == -15)
				|| (u.getScale() == -18);
		if (u.getOffset() != 0d) {
			buffer.append(format(u.getOffset()).toString().replaceAll("\\$", ""));
			if ((u.getMultiplier() != 0) || (!standardScale)) buffer.append('+');
		}
	...
	public StringBuffer format(UnitDefinition def) {
		...
		for (int j = 0; j < def.getNumUnits(); j++) {
			...
			if (j < def.getListOfUnits().size() - 1) buffer.append("\\cdot ");
		}
		return buffer;
	}
	...
	public void setHeadingsFont(String fontHeadings) {
		...
		if (fontHeadings.equals("helvetica")) fontHeadings = "helvet";
		...
		else System.err.println("Unsupported font " + fontHeadings + ". Using "
				+ this.fontHeadings + ".");
	}
	...
	private void documentFoot(SBMLDocument doc, BufferedWriter buffer)
		...
		if (checkConsistency) {
			...
			if (doc.getNumErrors() > 0) {
				...
				buffer.newLine();
				if (xml.size() > 0)
					problemMessage(xml, doc, xml.size() > 1 ? "XML errors" : "XML error",
						buffer, "Error");
				if (fatal.size() > 0)
				...
				if (infos.size() > 0)
					problemMessage(infos, doc, infos.size() > 1 ? "Information messages"
							: "Information message", buffer, "Information");
				if (warnings.size() > 0)
		...
		if (sboTerms.size() > 0) {
			...
			Arrays.sort(sbo);
			for (int id : sbo)
				buffer.append(descriptionItem("SBO:" + SBO.sboNumberString(id),
					"\\textbf{"
							+ maskSpecialChars(
								correctQuotationMarks(SBO.getTerm(id).getName(),
									leftQuotationMark, rightQuotationMark))
									.replaceAll("\\n", " ")
							+ ":} "
							+ maskSpecialChars(
								correctQuotationMarks(SBO.getTerm(id).getDescription(),
									leftQuotationMark, rightQuotationMark))
									.replaceAll("\\n", " ")));
			buffer.append(descriptionEnd);
		}
	...
	private void documentHead(SBMLDocument doc, BufferedWriter buffer)
		...
		buffer.newLine();
		buffer
				.append(space + "pdfauthor={Produced by SBML2LaTeX version 1.0beta},");
		buffer.newLine();
	...
	private void format(CVTerm cv, BufferedWriter buffer) throws IOException {
		...
		for (int i = 0; i < resources.size(); i++) {
			...
			if (urls != null) {
				buffer.append(item);
				if (urls.length >= 1) {
					buffer.append(href(urls[0].replaceAll("\\%", "\\\\%"), texttt(
						identifier).toString()));
					buffer.append('.');
				} else buffer.append(" no URL available for resource identifier "
						+ texttt(identifier));
			}
			buffer.newLine();
		}
		...
		if ((def.getNumArguments() > 0) || (def.getBody() != null)
			...
			if (def.getNumArguments() > 0) {
				...
				buffer.append(descriptionItem("Mathematical Formula", equation(
					getNameOrID(def, true), new StringBuffer(def.getMath().toLaTeX()))));
			}
		}
		buffer.append(descriptionEnd);
	}
	...
	private void formatReactions(ListOf<? extends SBase> reactionList,
		...
		if (model.getNumSpecies() > 0) {
			...
			for (speciesIndex = 0; speciesIndex < model.getNumSpecies(); speciesIndex++) {
				speciesIDandIndex.put(model.getSpecies(speciesIndex).getId(), Integer
						.valueOf(speciesIndex));
			...
			for (speciesIndex = 0; speciesIndex < model.getNumSpecies(); speciesIndex++) {
				...
				if (species.isSetInitialConcentration()) {
					...
					if ((model.getUnitDefinition("substance") != null)
						...
						if (species.isSetSubstanceUnits()
							...
							if (Unit.isUnitKind(species.getSubstanceUnits(), species
									.getLevel(), species.getVersion())) {
								Unit u = new Unit(species.getLevel(), species.getVersion());
								u.setKind(JSBML.forName(species.getSubstanceUnits()));
								ud.addUnit(u);
							}
							// else: something's wrong.
						} else {
							ud = new UnitDefinition(species.isSetSubstanceUnits() ? model
									.getUnitDefinition(species.getSubstanceUnits()) : model
				...
				if (species.isSetCharge()) {
					buffer.append(descriptionItem("Charge", Integer.toString(species
							.getCharge())));
				}
				if (species.isSetSpeciesType()) {
					SpeciesType type = model.getSpeciesType(species.getSpeciesType());
					StringBuffer text = new StringBuffer(texttt(maskSpecialChars(type
							.getId())));
					if (type.isSetName()) {
						text.append(" (");
				...
				for (i = 0; (i < model.getNumInitialAssignments()); i++) {
					hasInitialAssignment = model.getInitialAssignment(i).getSymbol()
				...
				if (hasInitialAssignment) {
					buffer.append(descriptionItem("Initial assignment ", Integer
							.toString(i)));
				}
	...
	private StringBuffer format(Reaction r, int reactionIndex)
		...
		if (arrangeReactionParticipantsInOneTable) {
			...
			for (i = 0; i < Math.max(r.getNumReactants(), Math.max(
				r.getNumProducts(), r.getNumModifiers())); i++) {
				Species s;
				if (r.getNumReactants() > 0) {
		...
		if (((sBase instanceof NamedSBase) && ((NamedSBase) sBase).isSetName())
			...
			if (sBase.isSetSBOTerm()) {
				buffer.append(descriptionItem("SBO:"
						+ SBO.sboNumberString(sBase.getSBOTerm()), correctQuotationMarks(
					SBO.getTerm(sBase.getSBOTerm()).getName(), leftQuotationMark,
					rightQuotationMark)));
	 ...
	 * @throws IOException
	 */
	private void formatVelocity(Integer k, boolean notSubstancePerTimeUnit,
		boolean notExistingKineticLaw, BufferedWriter equationBW)
		throws IOException {
		StringBuffer v = new StringBuffer("v_{");
...
//		imprint.append(newLine());
		...
		imprint.append(newLine());
		imprint.append("\\bibitem[Dr\\\"ager {\\em et~al.}(2009)Dr\\\"ager, Planatscher, Wouamba, Schr\\\"oder,");
		imprint.append(newLine());
		imprint.append("  Hucka, Endler, Golebiewski, M{\\\"u}ller, and Zell]{Draeger2009b}");
		imprint.append(newLine());
		imprint.append("Dr\\\"ager, A., Planatscher, H., Wouamba, D.~M., Schr\\\"oder, A., Hucka, M.,");
		imprint.append(newLine());
		imprint.append("  Endler, L., Golebiewski, M., M{\\\"u}ller, W., and Zell, A. (2009).");
		imprint.append(newLine());
	...
	private void problemMessage(Vector<Integer> listOfErrorIndices,
		...
		for (Integer e : errors) {
			...
			buffer.append(descriptionItem(messageType + ' '
					+ Integer.toString(error.getErrorId()), message.toString()));
		}
		buffer.append(descriptionEnd);
		buffer.newLine();
	}
			...
			for (i = 0; i < r.getNumReactants(); i++) {
				if (r.getReactant(i) == null) {
					...
					reactString.append(math(getNameOrID(r.getModel().getSpecies(
						r.getReactant(i).getSpecies()), true)));
				}
	...
	private StringBuffer unitTest(String kind, Model model) {
		if (Unit.isUnitKind(kind, model.getLevel(), model.getVersion())) {
			...
			u.setKind(JSBML.forName(kind));
			return format(u);
		} else if (Unit.isPredefined(kind, model.getLevel())) {
			Unit u = new Unit(model.getLevel(), model.getVersion());
			u.setKind(JSBML.forName(kind));
			return format(Unit.convertToSI(u));
		}
