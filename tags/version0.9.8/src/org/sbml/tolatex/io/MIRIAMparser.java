/*
 * $Id: MIRIAMparser.java 60 2011-03-07 17:20:39Z draeger $
 * $URL: https://rarepos.cs.uni-tuebingen.de/svn/SBML2LaTeX/tags/version0.9.8/src/org/sbml/tolatex/io/MIRIAMparser.java $
 * ---------------------------------------------------------------------
...
public class MIRIAMparser {
	...
	if (uri.startsWith("urn:")) {
	    ...
	    int beginIndex = Math.min(uri.lastIndexOf('/') + 1, uri
		    .indexOf('#'));
	    if ((beginIndex < 0) || (uri.length() <= beginIndex)) {
		beginIndex = Math.max(0, uri.length() - 1);
	    }
	    id = uri.substring(beginIndex);
	    miriamURI.append(uri.substring(0, uri.length() - id.length()));
	    if (id.startsWith("#")) {
		id = id.substring(1);
	    }
	 ...
	 * @param uri
	 *        Some URI (urn, uri)
	 * @return An array of valid urls if these can be found from the XML file. An
	 *         empty array (length 0) otherwise.
	 */
	public String[] getLocations(String uri) {
		...
		Element datatype = identifyDatatype(miriamURI.toString(), new StringBuffer());
		if (datatype == null) {
		...
		if (datatype != null) {
			...
			for (Iterator<?> iter = datatype.getDescendants(new ElementFilter(
				...
				Element entry = resource.getChild("dataEntry", resource.getNamespace());
				if (entry != null) {
		...
		for (Iterator<?> iter = doc.getDescendants(new ElementFilter("uris")); iter
			...
			for (int i = 0; i < uris.getChildren().size(); i++) {
				...
				if (child.getAttribute("type") != null) {
					if (child.getAttribute("type").getValue().equalsIgnoreCase("urn")) {
						if (child.getAttributeValue("deprecated") != null) {
							if (!Boolean.parseBoolean(child.getAttributeValue("deprecated"))) {
								urnString = child.getValue();
							} else if (urnString.length() == 0) {
								urnString = child.getValue();
							}
						} else if (urnString.length() == 0) urnString = child.getValue();
					}
				}
	 ...
	 * @throws IOException
	 */
	public void setMIRIAMDocument(InputStream stream) throws JDOMException,
		IOException {
