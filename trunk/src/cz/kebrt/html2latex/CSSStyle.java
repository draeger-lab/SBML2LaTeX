...
class CSSStyle {
   ...
   */
  private static final Logger logger = Logger.getLogger(CSSStyle.class.getName());
  ...
  public void makeLaTeXCommands(Configuration conf) {
    ...
    for (Iterator<Map.Entry<String, String>> iterator = _properties.entrySet().iterator(); iterator.hasNext();) {
  ...
  private void colorProperty(Configuration conf) {
    ...
    if (color.startsWith("#")) {
      ...
      try {
        ...
        logger.warning("Wrong color definition in style: " + _name);
        return;
      }
